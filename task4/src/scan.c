...
int init_scan(char *filename){ /* Initialize */
	...
	if((newfile = (char *)malloc(strlen(filename))) == NULL){
		fprintf(stderr, "can not malloc in init_scan\n");
		return -1;
	}
	...
	sprintf(newfile,"%s.csl",newfile);

	if((fpw = fopen(newfile, "w")) == NULL){
		fprintf(stderr, "csl file can not open");
		return -1;
	}
